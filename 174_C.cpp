 ...
 *	Created: Tuesday 16 March 2021 04:22:23 PM IST
**/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int32_t main() {
	...
	auto find_first_positive = [&]() {
		int lo = 1, hi = n;
		int ret = -1;
		while (lo <= hi) {
			int mid = (lo + hi) >> 1;
			if (T.query(1, mid).mx > 0) ret = mid, hi = mid - 1;
			else lo = mid + 1;
		}
		assert(~ret);
		return ret;
	...
	auto find_endpoint = [&](int from) {
		int lo = 1, hi = n;
		int ret = -1;
		while (lo <= hi) {
			int mid = (lo + hi) >> 1;
			if (T.query(from, mid).mn > 0) ret = mid, lo = mid + 1;
			else hi = mid - 1;
		}
		assert(~ret);
		return ret;
	...
	while (T.query(1,n).mx != 0) {
		int from = find_first_positive();
		int to = find_endpoint(from);
		answers.emplace_back(from, to);
		T.update(from, to, -1);
	}
	assert(sz(answers) <= 100000);
	cout << sz(answers) << '\n';
	for (auto [L, R] : answers) {
		cout << L << ' ' << R << '\n';
	}
}
