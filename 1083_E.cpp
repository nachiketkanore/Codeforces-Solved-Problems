...
(ツ) He who wishes to secure the good of others, has already secured his own.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int32_t main() {
	...
	for (auto &it : a) for (auto &it2 : it) cin >> it2;
	sort(ALL(a));
	vector<int> dp(n);
	ConvexHullDynamic lines(true);
	int answer = 0;
	for (int i = 0; i < n; i++) {
		auto [x, y, cost] = a[i];
		dp[i] = x * y - cost;
		if (i > 0) {
			int mx = lines.getBest(y);
			int best_here = x * y - cost + mx;
			dp[i] = max(dp[i], best_here);
		}
		lines.addLine(-x, dp[i]);
