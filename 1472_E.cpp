...
void solve() {
	...
	FOR (i,1,n) {
		one.update(1, mx, one.root, h[i], w[i], i);
		two.update(1, mx, two.root, w[i], h[i], i);
	}

	FOR (i,1,n) {
		pair<int,int> best1 = one.query(1, mx, one.root, 1, h[i] - 1);
		pair<int,int> best2 = two.query(1, mx, two.root, 1, h[i] - 1);
