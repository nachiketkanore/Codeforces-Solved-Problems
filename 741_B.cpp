...
const int inf = 3e18;
const int N = 1234;
int n, m, want, b[N], w[N], vis[N], dp[N][N], wcomp[N], bcomp[N],cc;
vi g[N], comp[N];
...
int solve(int i, int curr){
   ...
   ans = solve(i + 1, curr);
   ans = max(ans, bcomp[i] + solve(i+1, curr + wcomp[i]));

   for(int single : comp[i]){
      ans = max(ans, b[single] + solve(i + 1, curr + w[single]));
   }
   return ans;

}

//Knapsack on graph - DSU
int32_t main(){ 
   FAST;
   cin >> n >> m >> want;
