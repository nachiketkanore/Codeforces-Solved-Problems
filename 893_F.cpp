...
int root, n, a[N], depth[N], val[N], tin[N], tout[N], timer, tour[N];
vector<int> g[N];
...
{	
	...
	void build(int u, int tl, int tr) {
		if (tl == tr) {
			tree[u].emplace_back(depth[tour[tl]], val[tour[tl]]);
			see(tl, tr, tree[u]);
			return;
			// see("node ", u, " id = ", tl, depth[tour[tl]], val[tour[tl]]);
		} else {
			int mid = (tl + tr) >> 1;
			build(2*u, tl, mid);
