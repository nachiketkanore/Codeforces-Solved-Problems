...
bool ok(int val) {
	FOR (i,1,m) FOR (j,1,m) cnt[i][j] = 0;
	FOR (i,1,n) {
		vector<int> good_pos;
		FOR (j,1,m) 
			if (mat[i][j] >= val) 
				good_pos.pb(j);
		FOR (col1, 1, sz(good_pos)) {
			FOR (col2, col1+1, sz(good_pos)) {
				cnt[good_pos[col1-1]][good_pos[col2-1]]++;
				if (cnt[good_pos[col1-1]][good_pos[col2-1]] == 2) 
					return true;
			}
		}
	}
