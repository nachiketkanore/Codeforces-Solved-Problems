...
Created: Thursday 04 March 2021 07:37:58 PM IST
(ツ) To change ones life, start immediately, do it flamboyantly, no exceptions.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
const int N = 2e4 + 5;
int n, k, a[N], b[N], Q, type, q, L, R, d;
...
int32_t main() {
	...
	while (Q--) {
		...
		if (type == 1) {
			...
			cin >> L >> R >> d;
			FOR (i,L,R) b[i] = (b[i] + d) % MOD;
		}
		FOR (i,1,n) b[i] = (q - b[i] + MOD) % MOD;
		vector<int> all_answers = divide_conquer(1,n);
		see(sz(all_answers), k);
		cout << (k < sz(all_answers) ? all_answers[k] : 0) << '\n';
	}
}
